...
function main(config) {
  try {
    ...
    // 代理组 (Proxy Groups)
    const proxyGroups = createProxyGroups(newConfig.proxies, highQualityProxies);

    // 规则 (Rules)
    const rules = [...forceProxyDomains];

    // 规则提供者 (Rule Providers)
    const ruleProviders = createRuleProviders(rules);
...
function createProxyGroups(allProxies, highQualityProxies) {
  return [
    { name: "自动选择", icon: "https://raw.githubusercontent.com/Orz-3/mini/master/Color/Global.png", type: "url-test", "include-all": true, url: TEST_URL, interval: TEST_INTERVAL, tolerance: TEST_TOLERANCE, proxies: [...allProxies.map(p => p.name), ...highQualityProxies] },
    { name: "手动选择", icon: "https://raw.githubusercontent.com/Orz-3/mini/master/Color/SS.png", type: "select", proxies: ["自动选择", "高质量节点", "DIRECT"] },
    { name: "高质量节点", icon: "https://raw.githubusercontent.com/Orz-3/mini/master/Color/Speedtest.png", type: "select", proxies: ["自动选择", "负载均衡", "DIRECT", ...highQualityProxies] },
    { name: "负载均衡", icon: "https://raw.githubusercontent.com/Orz-3/mini/master/Color/Catnet.png", type: "load-balance", "include-all": true, strategy: BALANCESTRATEGY, url: TEST_URL, interval: TEST_INTERVAL },
    { name: "国内直连", icon: "https://raw.githubusercontent.com/Orz-3/mini/master/Color/China.png", type: "select", proxies: ["DIRECT"] },
    { name: "广告拦截", icon: "https://raw.githubusercontent.com/Orz-3/mini/master/Color/Adblock.png", type: "select", proxies: ["REJECT"] },
    { name: "终则", icon: "https://raw.githubusercontent.com/Orz-3/mini/master/Color/Shandian.png", type: "select", proxies: ["DIRECT", "自动选择"] }
  ];
}
